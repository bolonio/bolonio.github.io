...
But a lot of times we don’t think about the user experience for people with disabilities, including people with age-related impairments.
...
>
> Source: [The International Association of Accessibility Professionals (IAAP)]()

Why is so obvious in real world that the three situations below are completely wrong?.

![react-axe](./wrong-a11y.jpeg)

Probably because those three ramps were not build by disabled people, or maybe because they didn't put themselves in their shoes?
...
But it's very clear to me that these three situations are definetely wrongly done.
...
Let's see how can we translate this reality to the online world, because for some of us, not visually impaired people, it's not so obvious to identify a non-accessible website.
...
The problem here is that for visually impaired people such as blidn people, there's no such as thing as top right corner or button with the engine icon.
So we need to have these situations in mind when we develop a new website.

Based on number from the [World Health Organization (WHO)](https://www.who.int/news-room/fact-sheets/detail/blindness-and-visual-impairment),
...
I've separated this article in three parts: testing the code, testing the DOM, and testing manually in the browser.

#### Testing the code

I've created a very very small React application with three tiny components: a button, a fake button, which is an anchor link with the role button, and an image.
In the main application I've put a bunch of accessibility errors, so let's see how can I found them using some tools.
...
You need to be sure that this is only done in a development environtment and not in production.
...
![react-axe](./eslint-plugin-jsx-a11y.jpeg)
...
You can run the `axe` command in your CLI, followed by the URL of the page you wish to test.

![react-axe](./axe-cli.jpeg)

The same as with react-axe, each violation includes the description of the issue, the HTML element in where the violation was found, and a link to the [dequeuniversity](), a full documentation source with detailed information about the issue, and the steps to solve it.
...
![react-axe](./lighthouse.jpeg)

At the end of the analysis, the tools will generate a final report, either HTML or JSON/CSV the can be use for reporting purposes
...
You can find the extensions [here](https://www.deque.com/axe/axe-for-web/).

![react-axe](./axe-chrome.jpeg)
...
You can find the extension [here](https://www.paciellogroup.com/toolkit/).

![react-axe](./arc-chrome.jpeg)
...
You can find the extensions [here](https://accessibilityinsights.io/docs/en/web/overview).

![react-axe](./accessibility-insights.jpeg)
...
You can find the extensions [here](https://wave.webaim.org/extension/).

![react-axe](./wave-chrome.jpeg)

##### NoCoffee - Vision Simulator

An interesting tool is NoCoffee.
...
You can find the extension [here](https://accessgarage.wordpress.com/).

![react-axe](./nocoffee-chrome.jpeg)
...
We need to understand that the responsibility to create accessible apps doesn't belong to the developers or testers, but to the whole team, including PM or UX/UI designers.
...
![react-axe](./a11y-is-not-a-feature.jpeg)

If you have any question to ask, any recommendation to make, or any appreciation to give, please feel free to contact me.
The best way to reach me is either via my twitter at [@bolonio](www.twitter.com/bolonio) or you can send me an email at [adrian.bolonio@gmail.com](mailto:adrian.bolonio@gmail.com).
